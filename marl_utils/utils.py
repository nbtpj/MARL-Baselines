...
def mulbatch_from_list(dict_list: List[Dict[str, Union[Dict[str, np.ndarray], np.ndarray]]],
    ...
    for agent_id in agents:
        # fill
        # for list_id in range(total_id):
        #     if useoptional_action_mask[agent_id] and optional_action_mask[agent_id][list_id] is None:
        #         optional_action_mask[agent_id] = fill_value[agent_id]["action_mask"]
        #         collated[agent_id][list_id] = fill_value[agent_id]["observation"]
        #     elif collated[agent_id][list_id] is None:
        #         collated[agent_id][list_id] = fill_value[agent_id]
        # return data type
...
class Algorithm:
    ...
    @torch.no_grad
    def eval_fn(self, verbose: Optional[Callable[[int], Any]] = None,) -> Dict:
        """
        ...
        last_state_info = {agent_id: [None for _ in range(eval_rollout_group.n_worker)] 
                           for agent_id in self.agents}
        ...
        while not all(all(_done) for _done in _Done.values()):
            ...
            Obs, Rew, Ter, Trunc, Info = eval_rollout_group.step(actions=Act,
                                                                    log_probs=LogProb)
            is_new_done = {
                agent_id: False for agent_id in eval_rollout_group.agents
            }
            # MulObs, MulBatched, MulBatched, MulBatched, List
            for agent_id in eval_rollout_group.agents:
                # or at done, we dont really care the batch mask because it is one-off
                _Rew[agent_id] += Rew[agent_id] * (1-_Done[agent_id].astype(float))
                _EpsLen[agent_id] += (1-_Done[agent_id].astype(float))
                _t = np.logical_or(np.logical_or(_Done[agent_id].astype(bool), 
                                                            Trunc[agent_id].data.astype(bool)), 
                                                    Ter[agent_id].data.astype(bool))
                is_new_done[agent_id] = np.logical_and(~_Done[agent_id], _t)
                _Done[agent_id] = _t
            # collect last info
            _, Info = eval_rollout_group.get_last_b4_reset()
            for p_id in range(eval_rollout_group.n_worker):
                for agent_id in eval_rollout_group.agents:
                    if is_new_done[agent_id][p_id] and agent_id in Info[p_id]:
                        last_state_info[agent_id][p_id] = Info[p_id][agent_id]
        return {
            'return': _Rew, 'eps_len': _EpsLen, "last_info": last_state_info
        }
